...
- 매우 낮은 성능의 기기에서 프레임 스킵이 발생할 때 가변 델타 타임을 사용하여 게임 로직이 일관되게 동작한다
...
- **FR-005**: 시스템은 캔버스 크기를 화면에 맞게 자동으로 조절하되, 비율을 유지하고 letterbox(검은 여백)를 적용해야 한다
- **FR-006**: 시스템은 게임 오브젝트를 재사용 가능한 풀에서 관리하여 메모리 할당을 최소화해야 한다
- **FR-007**: 시스템은 브라우저 탭이 비활성화될 때 자동으로 게임을 일시정지하고, 탭 복귀 시 일시정지 상태를 유지해야 한다
- **FR-008**: 시스템은 2D 그래픽 요소를 캔버스에 렌더링하는 기능을 제공해야 한다
- **FR-009**: 시스템은 토글 가능한 FPS 카운터를 제공하여 성능 모니터링을 지원해야 한다
...
- Q: 브라우저 탭 비활성화 시 동작 → A: 자동 일시정지 (탭 복귀 시 일시정지 상태 유지)
- Q: 프레임 드롭 발생 시 게임 로직 처리 방식 → A: 가변 델타 타임 (프레임 간 실제 경과 시간 기반)
- Q: 캔버스 리사이즈 전략 → A: 비율 유지 + letterbox (검은 여백)
- Q: 게임 기준 해상도 (디자인 해상도) → A: 720x1280 (HD)
- Q: 개발자/디버그 모드 포함 여부 → A: FPS 카운터만 표시 (토글 가능)

## Assumptions

- 대상 브라우저는 Chrome, Edge, Safari, Firefox의 최신 2개 버전이다
- 최소 지원 해상도는 320x480px (모바일 세로)이다
- 게임 기준 해상도(디자인 해상도)는 720x1280px (HD, 세로 모드)이다
